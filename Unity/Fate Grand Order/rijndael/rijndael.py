import copy
from .constants import shifts, r_con, num_rounds, S, Si, U1, U2, U3, U4, T5, T6, T7, T8
...
class Rijndael:
    def __init__(self, key, block_size: int = 16):
        ...
        for i in range(0, k_c):
            tk.append((ord(key[i * 4:i * 4 + 1]) << 24) | (ord(key[i * 4 + 1:i * 4 + 1 + 1]) << 16) | (ord(key[i * 4 + 2: i * 4 + 2 + 1]) << 8) | ord(key[i * 4 + 3:i * 4 + 3 + 1]))
        ...
        while t < round_key_count:
            ...
            tt = tk[k_c - 1]
            tk[0] ^= (S[(tt >> 16) & 0xFF] & 0xFF) << 24 ^ (S[(tt >> 8) & 0xFF] & 0xFF) << 16 ^ (S[tt & 0xFF] & 0xFF) << 8 ^ (S[(tt >> 24) & 0xFF] & 0xFF) ^ (r_con[r_con_pointer] & 0xFF) << 24
            r_con_pointer += 1
            if k_c != 8:
                ...
                for i in range(1, k_c // 2):
                    tk[i] ^= tk[i - 1]
                tt = tk[k_c // 2 - 1]
                tk[k_c // 2] ^= (S[tt & 0xFF] & 0xFF) ^ (S[(tt >> 8) & 0xFF] & 0xFF) << 8 ^ (S[(tt >> 16) & 0xFF] & 0xFF) << 16 ^ (S[(tt >> 24) & 0xFF] & 0xFF) << 24
        ...
        for r in range(1, rounds):
            for j in range(b_c):
                tt = k_d[r][j]
                k_d[r][j] = (U1[(tt >> 24) & 0xFF] ^ U2[(tt >> 16) & 0xFF] ^ U3[(tt >> 8) & 0xFF] ^ U4[tt & 0xFF])
        self.Kd = k_d

    def decrypt(self, cipher):
        if len(cipher) != self.block_size:
            raise ValueError('wrong block length, expected %s got %s' % (str(self.block_size), str(len(cipher))))
        ...
        # cipher to ints + key
        for i in range(b_c):
            t[i] = (ord(cipher[i * 4: i * 4 + 1]) << 24 | ord(cipher[i * 4 + 1: i * 4 + 1 + 1]) << 16 | ord(cipher[i * 4 + 2: i * 4 + 2 + 1]) << 8 | ord(cipher[i * 4 + 3: i * 4 + 3 + 1])) ^ k_d[0][i]
        # apply round transforms
        for r in range(1, rounds):
            for i in range(b_c):
                a[i] = (T5[(t[i] >> 24) & 0xFF] ^ T6[(t[(i + s1) % b_c] >> 16) & 0xFF] ^ T7[(t[(i + s2) % b_c] >> 8) & 0xFF] ^ T8[t[(i + s3) % b_c] & 0xFF]) ^ k_d[r][i]
            t = copy.copy(a)
...
class RijndaelCbc(Rijndael):
    def __init__(self, key, iv, padding, block_size):
