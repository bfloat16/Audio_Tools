...
def _custom_load_folder(self, path):
    ...
    for root, dirs, files in self.fs.walk(path):
        for fname in files:
            low = fname.lower()
            if low.endswith(".unity3d") and "story_storydata" in low:
...
def _try_decompress(data):
    buf = io.BytesIO(data)
    num1, uncompressed_size, _, num2 = struct.unpack('<IIII', buf.read(16))
    if num1 != 100 or num2 != 1:
        raise ValueError('invalid data')
    compressed = buf.read()
    return lz4.block.decompress(compressed, uncompressed_size=uncompressed_size)

def _patched_load_assets(self, assets, open_f):
    split_files = []
    for path in assets:
        m = reSplit.match(path)
        if m:
            base, _ = m.groups()
            if base in split_files:
                continue
            split_files.append(base)
            data = self._load_split_file(base)
            name = base
        else:
        ...
        self.load_file(data, name=name)

Environment.load_assets = _patched_load_assets
...
if __name__ == "__main__":
    ...
    parser.add_argument("--src", default=r"E:\Game_Dataset\jp.co.bandainamcoent.BNEI0242\RAW\a")
    parser.add_argument("--dst", default=r"E:\Game_Dataset\jp.co.bandainamcoent.BNEI0242\EXP\Story")
