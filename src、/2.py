...
for entry in index_data:
    ...
    if audio_file_path:
        ...
        speaker_folder = os.path.join(ba.replace('vo', 'f'), speaker)
        if not os.path.exists(speaker_folder):
            os.makedirs(speaker_folder)
        # 移动音频文件到新的文件夹并添加 .ogg 后缀
        new_audio_file_path = os.path.join(speaker_folder, f"{audio_file_name}.ogg")
...
# 保存更新后的 index.json
output_index_path = os.path.join(ba.replace('vo', 'f'), 'index.json')
