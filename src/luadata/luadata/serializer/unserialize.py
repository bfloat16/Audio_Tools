...
def unserialize(raw, encoding="utf-8", multival=False, verbose=False):
    ...
    while pos <= slen:
        ...
        if pos < slen:
            ...
            byte_current_is_space = (byte_current == b" " or byte_current == b"\r" or byte_current == b"\n" or byte_current == b"\t")
        if verbose:
        ...
        if comment == "MULTILINE":
            ...
            if byte_current == b"-" and sbins[pos : pos + 4] == b"--[[":
                ...
                pos += 1
                pos1 = pos + 1
            elif not node["is_root"] and ((byte_current >= b"A" and byte_current <= b"Z") or (byte_current >= b"a" and byte_current <= b"z") or byte_current == b"_"):
                state = "KEY_SIMPLE"
                pos1 = pos
                ...
                if prev_env["state"] == "KEY_EXPRESSION_OPEN":
                    ...
                    node_entries_append(prev_env["node"], prev_env["key"], node_to_table(node))
                    state = "VALUE_END"
                    key = None
                node = prev_env["node"]
            ...
            elif byte_current == b"-" or (byte_current >= b"0" and byte_current <= b"9"):
                state = "INT"
                component_name = "VALUE"
                pos1 = pos
            elif byte_current == b".":
            ...
            if escaping:
                ...
                data = sbins[pos1:pos].replace(b"\\\n", b"\n").replace(b'\\"', b'"').replace(b"\\\\", b"\\").decode(encoding)
        ...
        elif state == 'TEXT_MULTILINE':
            if pos + 2 <= slen and sbins[pos : pos + 2] == b"]]":
                # We found the closing ]]
                data = sbins[pos1 : pos].decode(encoding)
                ...
                if component_name == "KEY":
                    ...
                    state = "VALUE_END"
                    key = None
                pos += 1  # Skip the extra ']'
            # No else: we keep scanning until we find ]]
        pos += 1
        if verbose:
    ...
    if errmsg is not None:
        ...
        err_parts = sbins[start_pos:end_pos]
